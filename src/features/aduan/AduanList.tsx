        ...
        <div className="space-y-6 animate-fade-in pb-20">
            ...
            <div className="flex flex-col gap-6">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-white p-4 rounded-card border border-slate-100 shadow-premium border-l-4 border-l-brand-500 text-center transition-all hover:shadow-md">
                        <p className="text-caption font-medium mb-1">Total Masuk</p>
                        <p className="text-xl font-bold text-slate-900">{stats?.total || 0}</p>
                    </div>
                    <div className="bg-white p-4 rounded-card border border-slate-100 shadow-premium border-l-4 border-l-amber-500 text-center transition-all hover:shadow-md">
                        <p className="text-caption font-medium mb-1">Menunggu</p>
                        <p className="text-xl font-bold text-slate-900">{stats?.pending || 0}</p>
                    </div>
                    <div className="bg-white p-4 rounded-card border border-slate-100 shadow-premium border-l-4 border-l-blue-500 text-center transition-all hover:shadow-md">
                        <p className="text-caption font-medium mb-1">Diproses</p>
                        <p className="text-xl font-bold text-slate-900">{stats?.processing || 0}</p>
                    </div>
                    <div className="bg-white p-4 rounded-card border border-slate-100 shadow-premium border-l-4 border-l-brand-600 text-center transition-all hover:shadow-md">
                        <p className="text-caption font-medium mb-1">Selesai</p>
                        <p className="text-xl font-bold text-slate-900">{stats?.completed || 0}</p>
                    </div>
                </div>

                <div className="flex bg-slate-100/50 p-1 rounded-btn w-fit self-center md:self-start border border-slate-200">
                    <button 
                        onClick={() => setViewTab('list')}
                        className={`px-6 py-2 rounded-btn text-xs font-bold transition-all flex items-center gap-2 ${viewTab === 'list' ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <Users weight="bold" /> Daftar Aspirasi
                    </button>
                    <button 
                        onClick={() => setViewTab('dashboard')}
                        className={`px-6 py-2 rounded-btn text-xs font-bold transition-all flex items-center gap-2 ${viewTab === 'dashboard' ? 'bg-white text-brand-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
                    >
            ...
            {viewTab === 'dashboard' ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="bg-white p-6 rounded-card border border-slate-100 shadow-premium">
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 pb-2 overflow-x-auto no-scrollbar">
                        <select 
                            ...
                            className="bg-white border border-slate-200 rounded-btn px-4 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-brand-500 transition-all text-slate-600"
                        >
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {isLoading ? (
                            ...
                            <div className="bg-white border-2 border-dashed border-slate-200 rounded-card p-16 text-center flex flex-col items-center gap-4">
                            ...
                            items.map((item) => (
                                <div key={item.id} className="bg-white rounded-card border border-slate-100 shadow-premium hover:shadow-md transition-all duration-300 overflow-hidden flex flex-col md:flex-row">
                                    ...
                                    <div className="p-5 flex-1 space-y-4">
                                        ...
                                        <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-100">
                                            <p className="text-body">{item.deskripsi}</p>
                                        </div>
                                        ...
                                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center pt-2 gap-4">
                                            ...
                                            <div className="flex items-center gap-2 w-full md:w-auto">
                                                <button 
                                                    ...
                                                    className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-btn text-xs font-bold hover:bg-slate-50 transition-all shadow-sm"
                                                >
                                                ...
                                                {isPengurus && item.tipe === 'Usulan' && item.status !== 'Selesai' && !item.polling && (
                                                    <button 
                                                        ...
                                                        className="flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-1.5 bg-brand-50 text-brand-600 border border-brand-100 rounded-btn text-xs font-bold hover:bg-brand-100 transition-all shadow-sm"
                                                    >
                                                ...
                                                <HasPermission module="Aduan & Usulan" action="Hapus">
                                                    <button 
                                                        ...
                                                        className="p-1.5 text-slate-400 hover:text-rose-500 bg-slate-50 rounded-btn transition-all"
                                                    >
                                                        <Trash weight="bold" />
                                                    </button>
                                                </HasPermission>
            ...
            {selectedItem && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
                    <div className="bg-white rounded-card w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200 overflow-hidden">
